...
- `publishing/` — real Cloudflare Pages integration, wired to a real dashboard button ("Öffentlich bereitstellen"). `CloudflarePagesPublisher` shells out to the locally-installed `wrangler` CLI (`pages project create` then `pages deploy <dir>`) — Cloudflare documents no public REST API for multi-file Direct Upload, only Wrangler and dashboard drag-and-drop, and the whole demo directory (index.html + assets/) needs uploading, not just the HTML. One Pages project per lead slug → `https://<slug>.pages.dev`. `publishDemoPublicly()` only persists `Demo.publicUrl` after independently polling the URL until it's reachable over HTTPS *and* its content contains the lead's company name (Cloudflare Pages deployments propagate asynchronously, and a wrong/stale page would otherwise pass a bare reachability check). Fails closed with a clear error when Cloudflare credentials aren't in `.env` — see the Settings page for exact setup steps. The spawn mechanics (binary resolution, Windows shell quoting) are verified against the real Cloudflare API using a deliberately invalid token; an actual deploy has not yet been exercised against a live account.
